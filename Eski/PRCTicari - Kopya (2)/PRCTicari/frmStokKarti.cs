...
namespace PRCTicari
{
    public partial class frmStokKarti : Form
    {
        ...
        private void txtStokKodu_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Return)
            {
                if (!string.IsNullOrEmpty(txtStokKodu.Text.Trim()))
                {
                    ...
                    cmd.CommandText = "SELECT SBT.Bagli_Stok_No AS Stok_No, ST.Stok_Kodu AS Stok_Kodu, '...' AS Sec_Stok_Kodu, ST.Stok_Adi FROM Stok_Bagli_Tanitimi AS SBT INNER JOIN Stok_Tanitimi AS ST ON ST.Silindi = 0 AND ST.Stok_No = SBT.Bagli_Stok_No WHERE SBT.Stok_No = @Stok_No ORDER BY SBT.Sira_No";
                    cmd.Parameters.AddWithValue("@Stok_No", lblStokNo.Text.Trim());
        ...
        private void tsbKaydet_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "DELETE FROM Stok_Bagli_Tanitimi WHERE Stok_No = @Stok_No";
            cmd.Parameters.AddWithValue("@Stok_No", lblStokNo.Text.Trim());
            cmd.ExecuteNonQuery();
            cmd.Parameters.Clear();

            cmd.CommandText = "INSERT INTO Stok_Bagli_Tanitimi (Stok_No, Sira_No, Bagli_Stok_No) VALUES (@Stok_No, @Sira_No, @Bagli_Stok_No)";
