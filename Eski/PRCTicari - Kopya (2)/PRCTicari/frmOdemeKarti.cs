...
namespace PRCTicari
{
    public partial class frmOdemeKarti : Form
    {
        ...
        private int fncYeniOdemeNoGetir()
        {
            ...
            cmd.CommandText = "SELECT ISNULL(MAX(Odeme_No), CAST(0 AS INT)) + 1 AS Odeme_No FROM Odeme_Tanitimi";
        ...
        private void prcdKayitKoduBilgiGetir()
        {
            ...
            if (!string.IsNullOrEmpty(txtKayitKodu.Text))
            {
                ...
                if (cbKayitTipi.SelectedIndex == 1)
                    cmd.CommandText = "SELECT Cari_No AS Kayit_No, Unvani AS Kayit_Adi FROM Cari_Tanitimi WHERE Silindi = 0 AND Cari_Kodu = @Kayit_Kodu";
                else if (cbKayitTipi.SelectedIndex == 2)
                    cmd.CommandText = "SELECT CAST(0 AS INT) AS Kayit_No, Kasa_Adi AS Kayit_Adi FROM Kasa_Tanitimi WHERE Silindi = 0 AND Kasa_Kodu = @Kayit_Kodu";
        ...
        private void tsbKaydet_Click(object sender, EventArgs e)
        {
            ...
            if (blnYeniKayit)
            {
                ...
                cmd.CommandText = "INSERT INTO Odeme_Tanitimi (Odeme_No, Odeme_Adi, Odeme_Tipi, Kayit_Tipi, Kayit_Kodu, Insert_User, Insert_Date, Odeme_Kodu) " +
                                  "VALUES (@Odeme_No, @Odeme_Adi, @Odeme_Tipi, @Kayit_Tipi, @Kayit_Kodu, @Kullanici, @Zaman, @Odeme_Kodu)";
            }
            else
                cmd.CommandText = "UPDATE Odeme_Tanitimi SET Odeme_No = @Odeme_No, Odeme_Adi = @Odeme_Adi, Odeme_Tipi = @Odeme_Tipi, Kayit_Tipi = @Kayit_Tipi, Kayit_Kodu = @Kayit_Kodu, Update_User = @Kullanici, Update_Date = @Zaman " +
                                  "WHERE Silindi = 0 AND Odeme_Kodu = @Odeme_Kodu";
        ...
        private void tsbSil_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bu kaydı silmek istediğinize emin misiniz?", "Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                cnn.Open();
                SqlCommand cmd = cnn.CreateCommand();
                cmd.CommandText = "UPDATE Odeme_Tanitimi SET Silindi = 1, Delete_User = @Delete_User, Delete_Date = @Delete_Date WHERE Silindi = 0 AND Odeme_Kodu = @Odeme_Kodu";
