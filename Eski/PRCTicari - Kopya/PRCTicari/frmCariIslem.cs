    ...
    public partial class frmCariIslem : Form
    {
        ...
        public frmCariIslem(clsFisTipleri.FisTipleri ftGetFisTipi)
        {
            ...
            if (iyIslemYonu == clsFisTipleri.IslemYonu.Giris || iyIslemYonu == clsFisTipleri.IslemYonu.Cikis)
            {
                ...
                {
                    colBorcTutariCari.Visible = false;
                    colAlacakTutariCari.HeaderText = "Tutarı";
                }
                else if (iyIslemYonu == clsFisTipleri.IslemYonu.Cikis)
                {
                    colAlacakTutariCari.Visible = false;
                    colBorcTutariCari.HeaderText = "Tutarı";
                }
            ...
            if (e.KeyCode == Keys.Return)
            {
                if (cbIsyeriKodu.SelectedItemForCode().TOINT() != 0)
                {
                    if (txtFisNo.Text.Trim().TOINT() != 0)
                    {
                        ...
                        cmd.CommandText = "SELECT ID.Cari_No, CT.Cari_Kodu, '...' AS Sec_Cari_Kodu, CT.Unvani, ID.Alacak_Tutari_Cari, ID.Borc_Tutari_Cari, ID.Aciklama " +
        ...
        private void dgvKalemler_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvKalemler.Columns[e.ColumnIndex].Name == colBorcTutariCari.Name ||
            dgvKalemler.Columns[e.ColumnIndex].Name == colAlacakTutariCari.Name)
            {
                if (dgvKalemler.Columns[e.ColumnIndex].Name == colBorcTutariCari.Name)
                    dgvKalemler.Rows[e.RowIndex].Cells[colAlacakTutariCari.Name].Value = 0;

                if (dgvKalemler.Columns[e.ColumnIndex].Name == colAlacakTutariCari.Name)
                    dgvKalemler.Rows[e.RowIndex].Cells[colBorcTutariCari.Name].Value = 0;
        ...
        {
            double dblBorcToplam = dtKalemler.Compute("SUM(Borc_Tutari_Cari)", "").TODOUBLE();
            double dblAlacakToplam = dtKalemler.Compute("SUM(Alacak_Tutari_Cari)", "").TODOUBLE();
        ...
        private bool fncKaydet()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "INSERT INTO Islem_Detay (Fis_Tarihi, Fis_Saati, Cari_No, Alacak_Tutari_Cari, Alacak_Tutari_Cari_Baslik, Alacak_Tutari_Cari_Kart, Alacak_Tutari_Cari_Sistem, Borc_Tutari_Cari, Borc_Tutari_Cari_Baslik, Borc_Tutari_Cari_Kart, Borc_Tutari_Cari_Sistem, Aciklama, Silindi, Fis_Tipi, Isyeri_Kodu, Fis_No, Fis_Sira, Islem_Yonu) " +
                                  "VALUES (@Fis_Tarihi, @Fis_Saati, @Cari_No, @Alacak_Tutari_Cari, @Alacak_Tutari_Cari_Baslik, @Alacak_Tutari_Cari_Kart, @Alacak_Tutari_Cari_Sistem, @Borc_Tutari_Cari, @Borc_Tutari_Cari_Baslik, @Borc_Tutari_Cari_Kart, @Borc_Tutari_Cari_Sistem, @Aciklama, @Silindi, @Fis_Tipi, @Isyeri_Kodu, @Fis_No, @Fis_Sira, @Islem_Yonu)";
                for (int iSira = 0; iSira < dtKalemler.Rows.Count; iSira++)
                {
                    DataRow DR = dtKalemler.Rows[iSira];

                    clsFisTipleri.IslemYonu iyIslemYonuS = (iyIslemYonu == clsFisTipleri.IslemYonu.Yok ? (DR["Borc_Tutari_Cari"].TODOUBLE() != 0 ? clsFisTipleri.IslemYonu.Cikis : clsFisTipleri.IslemYonu.Giris) : iyIslemYonu);
                    cmd.Parameters.AddWithValue("@Fis_Tarihi", dtpFisTarihi.Value.TODATE());
                    cmd.Parameters.AddWithValue("@Fis_Saati", dtpFisSaati.Value.TODATE());
                    cmd.Parameters.AddWithValue("@Cari_No", DR["Cari_No"].TOINT());
                    cmd.Parameters.AddWithValue("@Alacak_Tutari_Cari", DR["Alacak_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Alacak_Tutari_Cari_Baslik", DR["Alacak_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Alacak_Tutari_Cari_Kart", DR["Alacak_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Alacak_Tutari_Cari_Sistem", DR["Alacak_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Borc_Tutari_Cari", DR["Borc_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Borc_Tutari_Cari_Baslik", DR["Borc_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Borc_Tutari_Cari_Kart", DR["Borc_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Borc_Tutari_Cari_Sistem", DR["Borc_Tutari_Cari"].TODOUBLE());
                    cmd.Parameters.AddWithValue("@Aciklama", txtAciklama.Text.Trim());
                    cmd.Parameters.AddWithValue("@Silindi", 0);
                    cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                    cmd.Parameters.AddWithValue("@Isyeri_Kodu", cbIsyeriKodu.SelectedItemForCode());
