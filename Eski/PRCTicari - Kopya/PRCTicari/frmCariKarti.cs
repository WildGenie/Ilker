...
namespace PRCTicari
{
    public partial class frmCariKarti : Form
    {
        ...
        private int fncYeniCariNoGetir()
        {
            ...
            cmd.CommandText = "SELECT ISNULL(MAX(Cari_No), CAST(0 AS INT)) + 1 AS Cari_No FROM Cari_Tanitimi";
            ...
            if (e.KeyCode == Keys.Return)
            {
                if (!string.IsNullOrEmpty(txtCariKodu.Text.Trim()))
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        txtVergiNo.Text = reader["Vergi_No"].TOSTRING();
                    ...
                    cmd.CommandText = "SELECT CYT.Adi, CYT.Soyadi, CYT.Telefon_1, CYT.E_Mail_1 FROM Cari_Tanitimi AS CT INNER JOIN Cari_Yetkili_Tanitimi AS CYT ON CT.Cari_No = CYT.Cari_No WHERE CT.Silindi = 0 AND CT.Cari_No = @Cari_No";
        ...
        private void tsbKaydet_Click(object sender, EventArgs e)
        {
            ...
            if (blnYeniKayit)
            {
                ...
                cmd.CommandText = "INSERT INTO Cari_Tanitimi (Cari_No, Unvani, Grup_Kodu, Ozel_Kodu, Para_Birimi, Vergi_Dairesi, " +
                                  "Vergi_No, Musteri, Satici, Personel, Mahalle_Koy, Cadde_Sokak, Dis_Kapi_No, Site_Adi, Apartman_Blok, Kat, Daire, Ilce, Il, Ulke, " +
                                  "Telefon_1, Telefon_2, Telefon_3, Fax_1, Fax_2, E_Mail_1, E_Mail_2, Ise_Giris_Tarihi, Isten_Cikis_Tarihi, Gorev_Kodu, Maas, Insert_User, Insert_Date, Cari_Kodu) " +
                                  "VALUES (@Cari_No, @Unvani, @Grup_Kodu, @Ozel_Kodu, @Para_Birimi, @Vergi_Dairesi, " +
                                  "@Vergi_No, @Musteri, @Satici, @Personel, @Mahalle_Koy, @Cadde_Sokak, @Dis_Kapi_No, @Site_Adi, @Apartman_Blok, @Kat, @Daire, @Ilce, @Il, @Ulke, " +
                                  "@Telefon_1, @Telefon_2, @Telefon_3, @Fax_1, @Fax_2, @E_Mail_1, @E_Mail_2, @Ise_Giris_Tarihi, @Isten_Cikis_Tarihi, @Gorev_Kodu, @Maas, @Kullanici, @Zaman, @Cari_Kodu)";
            }
            else
                cmd.CommandText = "UPDATE Cari_Tanitimi SET Cari_No = @Cari_No, Unvani = @Unvani, Grup_Kodu = @Grup_Kodu, Ozel_Kodu = @Ozel_Kodu, Para_Birimi = @Para_Birimi, Vergi_Dairesi = @Vergi_Dairesi, " +
                                  "Vergi_No = @Vergi_No, Musteri = @Musteri, Satici = @Satici, Personel = @Personel, Mahalle_Koy = @Mahalle_Koy, Cadde_Sokak = @Cadde_Sokak, Dis_Kapi_No = @Dis_Kapi_No, Site_Adi = @Site_Adi, Apartman_Blok = @Apartman_Blok, Kat = @Kat, Daire = @Daire, Ilce = @Ilce, Il = @Il, Ulke = @Ulke, " +
                                  ...
                                  "WHERE Silindi = 0 AND Cari_Kodu = @Cari_Kodu";
            ...
            cmd.Parameters.AddWithValue("@Vergi_No", txtVergiNo.Text.Trim());
            ...
            cmd.CommandText = "DELETE FROM Cari_Yetkili_Tanitimi WHERE Cari_No = @Cari_No";
            cmd.Parameters.AddWithValue("@Cari_No", lblCariNo.Text.Trim());
            cmd.ExecuteNonQuery();
            cmd.Parameters.Clear();
            ...
            {
                cmd.CommandText = "INSERT INTO Cari_Yetkili_Tanitimi (Cari_No, Sira_No, Adi, Soyadi, Telefon_1, E_Mail_1) VALUES (@Cari_No, @Sira_No, @Adi, @Soyadi, @Telefon_1, @E_Mail_1)";
        ...
        private void tsbSil_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bu kaydı silmek istediğinize emin misiniz?", "Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                cnn.Open();
                SqlCommand cmd = cnn.CreateCommand();
                cmd.CommandText = "UPDATE Cari_Tanitimi SET Silindi = 1, Delete_User = @Delete_User, Delete_Date = @Delete_Date WHERE Silindi = 0 AND Cari_Kodu = @Cari_Kodu";
