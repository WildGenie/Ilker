        ...
        private void txtCariKodu_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Return)
            {
                if (!string.IsNullOrEmpty(txtCariKodu.Text.Trim()))
                {
                    ...
                    cmd.CommandText = "SELECT CYT.Adi, CYT.Soyadi, CYT.Telefon_1, CYT.E_Mail_1 FROM Cari_Tanitimi AS CT INNER JOIN Cari_Yetkili_Tanitimi AS CYT ON CT.Kurum_Kodu = CYT.Kurum_Kodu AND CT.Cari_No = CYT.Cari_No WHERE CT.Silindi = 0 AND CT.Kurum_Kodu = @Kurum_Kodu AND CT.Cari_No = @Cari_No";
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
