...
namespace PRCTicari
{
    public partial class frmKasaIslem : Form
    {
                ...
                if (cbIsyeriKodu.SelectedItemForCode().TOINT() != 0)
                {
                    if (txtFisNo.Text.Trim().TOINT() != 0)
                    {
                        ...
                        cmd.CommandText = "SELECT ID.Kasa_Kodu, '...' AS Sec_Kasa_Kodu, KT.Kasa_Adi, ID.Cari_No, CT.Cari_Kodu, '...' AS Sec_Cari_Kodu, ID.Masraf_Kodu, '...' AS Sec_Masraf_Kodu, MT.Masraf_Adi, CT.Unvani, Alacak_Tutari, Borc_Tutari, ID.Aciklama " +
                                          ...
                                          "LEFT OUTER JOIN Kasa_Tanitimi AS KT ON KT.Silindi = 0 AND KT.Kurum_Kodu = ID.Kurum_Kodu AND KT.Kasa_Kodu = ID.Kasa_Kodu " +
                                          "LEFT OUTER JOIN Cari_Tanitimi AS CT ON CT.Silindi = 0 AND CT.Kurum_Kodu = ID.Kurum_Kodu AND CT.Cari_No = ID.Cari_No " +
                                          "LEFT OUTER JOIN Masraf_Tanitimi AS MT ON MT.Silindi = 0 AND MT.Kurum_Kodu = ID.Kurum_Kodu AND MT.Masraf_Kodu = ID.Masraf_Kodu " +
        ...
        private void prcdKasaBilgiGetir()
        {
            ...
            cnn.Open();
            SqlCommand cmd = cnn.CreateCommand();
            cmd.CommandText = "SELECT * FROM Kasa_Tanitimi WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu AND Kasa_Kodu = @Kasa_Kodu";
            cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
        ...
        private void tsbSil_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bu fişi silmek istediğinize emin misiniz?", "Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                try
                {
                    cmd.CommandText = "UPDATE Islem_Baslik SET Silindi = @Silindi_Yeni, Delete_User = @Delete_User, Delete_Date = @Delete_Date WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    ...
                    cmd.ExecuteNonQuery();
                    cmd.Parameters.Clear();
                    cmd.CommandText = "UPDATE Islem_Detay_Cari SET Silindi = @Silindi_Yeni WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    cmd.Parameters.AddWithValue("@Silindi_Yeni", 1);
                    cmd.Parameters.AddWithValue("@Silindi", 0);
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                    cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                    ...
                    cmd.ExecuteNonQuery();
                    cmd.Parameters.Clear();
                    cmd.CommandText = "UPDATE Islem_Detay_Masraf SET Silindi = @Silindi_Yeni WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    cmd.Parameters.AddWithValue("@Silindi_Yeni", 1);
                    cmd.Parameters.AddWithValue("@Silindi", 0);
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                    cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
            ...
            try
            {
                ...
                for (int iSira = 0; iSira < dtKalemler.Rows.Count; iSira++)
                {
                    ...
                    cmd.CommandText = "INSERT INTO Islem_Detay_Kasa (Fis_Tarihi, Fis_Saati, Kasa_Kodu, Cari_No, Masraf_Kodu, Alacak_Tutari, Alacak_Tutari_Baslik, Alacak_Tutari_Kart, Alacak_Tutari_Sistem, Borc_Tutari, Borc_Tutari_Baslik, Borc_Tutari_Kart, Borc_Tutari_Sistem, Aciklama, Silindi, Kurum_Kodu, Fis_Tipi, Isyeri_Kodu, Fis_No, Fis_Sira, Islem_Yonu) " +
                                      "VALUES (@Fis_Tarihi, @Fis_Saati, @Kasa_Kodu, @Cari_No, @Masraf_Kodu, @Alacak_Tutari, @Alacak_Tutari_Baslik, @Alacak_Tutari_Kart, @Alacak_Tutari_Sistem, @Borc_Tutari, @Borc_Tutari_Baslik, @Borc_Tutari_Kart, @Borc_Tutari_Sistem, @Aciklama, @Silindi, @Kurum_Kodu, @Fis_Tipi, @Isyeri_Kodu, @Fis_No, @Fis_Sira, @Islem_Yonu)";
                    cmd.Parameters.AddWithValue("@Fis_Tarihi", dtpFisTarihi.Value.TODATE());
                    cmd.Parameters.AddWithValue("@Fis_Saati", dtpFisSaati.Value.TOTIME());
                    ...
                    {
                        cmd.CommandText = "INSERT INTO Islem_Detay_Cari (Fis_Tarihi, Fis_Saati, Cari_No, Alacak_Tutari, Alacak_Tutari_Baslik, Alacak_Tutari_Kart, Alacak_Tutari_Sistem, Borc_Tutari, Borc_Tutari_Baslik, Borc_Tutari_Kart, Borc_Tutari_Sistem, Aciklama, Silindi, Kurum_Kodu, Fis_Tipi, Isyeri_Kodu, Fis_No, Fis_Sira, Islem_Yonu) " +
                                          "VALUES (@Fis_Tarihi, @Fis_Saati, @Cari_No, @Alacak_Tutari, @Alacak_Tutari_Baslik, @Alacak_Tutari_Kart, @Alacak_Tutari_Sistem, @Borc_Tutari, @Borc_Tutari_Baslik, @Borc_Tutari_Kart, @Borc_Tutari_Sistem, @Aciklama, @Silindi, @Kurum_Kodu, @Fis_Tipi, @Isyeri_Kodu, @Fis_No, @Fis_Sira, @Islem_Yonu)";
                    ...
                    else if (!string.IsNullOrEmpty(DR["Masraf_Kodu"].TOSTRING()))
                    {
                        ...
                        cmd.CommandText = "INSERT INTO Islem_Detay_Masraf (Fis_Tarihi, Fis_Saati, Masraf_Kodu, Alacak_Tutari, Alacak_Tutari_Baslik, Alacak_Tutari_Kart, Alacak_Tutari_Sistem, Borc_Tutari, Borc_Tutari_Baslik, Borc_Tutari_Kart, Borc_Tutari_Sistem, Aciklama, Silindi, Kurum_Kodu, Fis_Tipi, Isyeri_Kodu, Fis_No, Fis_Sira, Islem_Yonu) " +
                                          "VALUES (@Fis_Tarihi, @Fis_Saati, @Masraf_Kodu, @Alacak_Tutari, @Alacak_Tutari_Baslik, @Alacak_Tutari_Kart, @Alacak_Tutari_Sistem, @Borc_Tutari, @Borc_Tutari_Baslik, @Borc_Tutari_Kart, @Borc_Tutari_Sistem, @Aciklama, @Silindi, @Kurum_Kodu, @Fis_Tipi, @Isyeri_Kodu, @Fis_No, @Fis_Sira, @Islem_Yonu)";
                        cmd.Parameters.AddWithValue("@Fis_Tarihi", dtpFisTarihi.Value.TODATE());
                        cmd.Parameters.AddWithValue("@Fis_Saati", dtpFisSaati.Value.TOTIME());
                        cmd.Parameters.AddWithValue("@Masraf_Kodu", DR["Masraf_Kodu"].TOSTRING());
                        cmd.Parameters.AddWithValue("@Alacak_Tutari", dblBorcTutari);
        ...
        private void btnGeri_Click(object sender, EventArgs e)
        {
            if (fncVazgec(false))
            {
                ...
                cnn.Open();
                SqlCommand cmd = cnn.CreateCommand();
                cmd.CommandText = string.Format("SELECT ISNULL({0}(Fis_No), (SELECT ISNULL({1}(Fis_No), CAST(1 AS INT)) FROM Islem_Baslik WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi_Ic AND Isyeri_Kodu = @Isyeri_Kodu_Ic)) AS Fis_No FROM Islem_Baslik WHERE Silindi = 0 AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No {2} @Fis_No",
