...
namespace PRCTicari
{
    public partial class frmKasaKarti : Form
    {
        ...
        private void tsbKaydet_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = cnn.CreateCommand();
            if (blnYeniKayit)
                cmd.CommandText = "INSERT INTO Kasa_Tanitimi (Kasa_Adi, Yetkilisi, Para_Birimi, Isyeri_Kodu, Insert_User, Insert_Date, Kurum_Kodu, Kasa_Kodu) " +
                                  "VALUES (@Kasa_Adi, @Yetkilisi, @Para_Birimi, @Isyeri_Kodu, @Kullanici, @Zaman, @Kurum_Kodu, @Kasa_Kodu)";
            else
                cmd.CommandText = "UPDATE Kasa_Tanitimi SET Kasa_Adi = @Kasa_Adi, Yetkilisi = @Yetkilisi, Para_Birimi = @Para_Birimi, Isyeri_Kodu = @Isyeri_Kodu, Update_User = @Kullanici, Update_Date = @Zaman " +
                                  "WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu AND Kasa_Kodu = @Kasa_Kodu";
        ...
        private void tsbSil_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bu kaydı silmek istediğinize emin misiniz?", "Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                cnn.Open();
                SqlCommand cmd = cnn.CreateCommand();
                cmd.CommandText = "UPDATE Kasa_Tanitimi SET Silindi = 1, Delete_User = @Delete_User, Delete_Date = @Delete_Date WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu AND Kasa_Kodu = @Kasa_Kodu";
