...
namespace PRCTicari
{
    public partial class rpStokEnvanter : Form
    {
        public rpStokEnvanter(string strFormID)
        {
            InitializeComponent();
            
            dtpTarihi.Value = DateTime.Now.Date;
        ...
        private void prcdRaporHazirla(byte bytTip)
        {
            ...
            cmd.CommandText = "SELECT @Fis_Tarihi_Bit AS Rapor_Tarihi, IDS.Fis_Tarihi, IDS.Fis_Saati, ST.Stok_Kodu, ST.Stok_Adi, ST.Birim_Kodu_1 AS Birim_Kodu, IDS.Giris_Miktari * (CASE WHEN IDS.Birim_Kodu = ST.Birim_Kodu_2 THEN ST.BT2_Orani ELSE CAST(1 AS INT) END) AS Giris_Miktari, IDS.Cikis_Miktari * (CASE WHEN IDS.Birim_Kodu = ST.Birim_Kodu_2 THEN ST.BT2_Orani ELSE CAST(1 AS INT) END) AS Cikis_Miktari " +
            ...
            cmd.Parameters.AddWithValue("@Fis_Tarihi_Bit", dtpTarihi.Value.ENDOFTHEDAY());
