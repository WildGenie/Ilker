    ...
    public partial class frmStokIslem : Form
    {
        ...
        private void prcdDetayGetir()
        {
            ...
            cmd.CommandText = "SELECT ID.Stok_No, ST.Stok_Kodu, '...' AS Sec_Stok_Kodu, ST.Stok_Adi, ID.Giris_Miktari, ID.Cikis_Miktari, ID.Birim_Kodu, " +
                              ...
                              "LEFT OUTER JOIN Stok_Tanitimi AS ST ON ST.Silindi = 0 AND ST.Kurum_Kodu = ID.Kurum_Kodu AND ST.Stok_No = ID.Stok_No " +
                ...
                ftFisTipi == clsFisTipleri.FisTipleri.IrsaliyeAlis || ftFisTipi == clsFisTipleri.FisTipleri.IrsaliyeSatis || ftFisTipi == clsFisTipleri.FisTipleri.IrsaliyeAlisIade || ftFisTipi == clsFisTipleri.FisTipleri.IrsaliyeSatisIade)
            {
                ...
                if (ftFisTipi == clsFisTipleri.FisTipleri.FaturaAlis || ftFisTipi == clsFisTipleri.FisTipleri.FaturaSatis || ftFisTipi == clsFisTipleri.FisTipleri.FaturaAlisIade || ftFisTipi == clsFisTipleri.FisTipleri.FaturaSatisIade)
                    cmd.CommandText = "SELECT Irsaliye_Fis_Tipi, Irsaliye_Isyeri_Kodu, Irsaliye_Fis_No, " +
                                      "(SELECT Belge_No FROM Islem_Baslik WHERE Kurum_Kodu = SBI.Kurum_Kodu AND Fis_Tipi = SBI.Irsaliye_Fis_Tipi AND Isyeri_Kodu = SBI.Irsaliye_Isyeri_Kodu AND Fis_No = SBI.Irsaliye_Fis_No) AS Irsaliye_No " +
                                      "FROM Stok_Bagli_Irsaliyeler AS SBI WHERE Kurum_Kodu = @Kurum_Kodu AND Fatura_Fis_Tipi = @Fis_Tipi AND Fatura_Isyeri_Kodu = @Isyeri_Kodu AND Fatura_Fis_No = @Fis_No";
                    ...
                    cmd.CommandText = "SELECT Fatura_Fis_Tipi, Fatura_Isyeri_Kodu, Fatura_Fis_No, " +
                                      "(SELECT Belge_No FROM Islem_Baslik WHERE Kurum_Kodu = SBI.Kurum_Kodu AND Fis_Tipi = SBI.Fatura_Fis_Tipi AND Isyeri_Kodu = SBI.Fatura_Isyeri_Kodu AND Fis_No = SBI.Fatura_Fis_No) AS Fatura_No " +
                                      "FROM Stok_Bagli_Irsaliyeler AS SBI WHERE Kurum_Kodu = @Kurum_Kodu AND Irsaliye_Fis_Tipi = @Fis_Tipi AND Irsaliye_Isyeri_Kodu = @Isyeri_Kodu AND Irsaliye_Fis_No = @Fis_No";
                cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                cmd.Parameters.AddWithValue("@Isyeri_Kodu", cbIsyeriKodu.SelectedItemForCode().TOINT());
                cmd.Parameters.AddWithValue("@Fis_No", txtFisNo.Text.Trim().TOINT());
            ...
            if (dgvKalemler.ReadOnly || string.IsNullOrEmpty(txtFaturaIrsaliyeNo.Text.Trim()))
            {
                if (MessageBox.Show("Bu fişi silmek istediğinize emin misiniz?", "Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        cmd.Parameters.Clear();
                        cmd.CommandText = "UPDATE Islem_Detay_Stok SET Silindi = @Silindi_Yeni WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                        cmd.Parameters.AddWithValue("@Silindi_Yeni", 1);
                        cmd.Parameters.AddWithValue("@Silindi", 0);
                        cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                        cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                        ...
                        cmd.ExecuteNonQuery();
                        cmd.Parameters.Clear();
                        cmd.CommandText = "DELETE FROM Stok_Bagli_Irsaliyeler WHERE Kurum_Kodu = @Kurum_Kodu AND Fatura_Fis_Tipi = @Fis_Tipi AND Fatura_Isyeri_Kodu = @Isyeri_Kodu AND Fatura_Fis_No = @Fis_No";
                        cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                        cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                        cmd.Parameters.AddWithValue("@Isyeri_Kodu", cbIsyeriKodu.SelectedItemForCode());
                        cmd.Parameters.AddWithValue("@Fis_No", txtFisNo.Text.TOINT());
        ...
        private bool fncKaydet()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "DELETE FROM Islem_Detay_Stok WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                cmd.Parameters.AddWithValue("@Silindi", 0);
                cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                cmd.Parameters.AddWithValue("@Isyeri_Kodu", cbIsyeriKodu.SelectedItemForCode());
                ...
                cmd.CommandText = "DELETE FROM Stok_Bagli_Irsaliyeler WHERE Kurum_Kodu = @Kurum_Kodu AND Fatura_Fis_Tipi = @Fatura_Fis_Tipi AND Fatura_Isyeri_Kodu = @Fatura_Isyeri_Kodu AND Fatura_Fis_No = @Fatura_Fis_No";
                cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                ...
                cmd.CommandText = "INSERT INTO Stok_Bagli_Irsaliyeler (Kurum_Kodu, Irsaliye_Fis_Tipi, Irsaliye_Isyeri_Kodu, Irsaliye_Fis_No, Fatura_Fis_Tipi, Fatura_Isyeri_Kodu, Fatura_Fis_No) VALUES (@Kurum_Kodu, @Irsaliye_Fis_Tipi, @Irsaliye_Isyeri_Kodu, @Irsaliye_Fis_No, @Fatura_Fis_Tipi, @Fatura_Isyeri_Kodu, @Fatura_Fis_No)";
                for (int i = 0; i < lstBagliIrsaliyeFisTipi.Count; i++)
                {
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
