...
namespace PRCTicari
{
    public partial class frmSecIrsaliye : Form
    {
        ...
        public void prcdCariBilgiGetir()
        {
            ...
            cmd.CommandText = "SELECT * FROM Cari_Tanitimi WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu AND Cari_No = @Cari_No";
        ...
        public void prcdIrsaliyeListesiGetir()
        {
            ...
            cmd.CommandText = "SELECT CAST(0 AS INT) AS Sec, IB.Fis_Tipi, IB.Isyeri_Kodu, IB.Fis_No, IB.Fis_Tarihi, IB.Belge_No, IB.Depo_Kodu_1, (CAST(IB.Depo_Kodu_1 AS VARCHAR) + '-' + DT.Depo_Adi) AS Depo_Adi, SUM(ABS(Giris_Tutari_Net - Cikis_Tutari_Net)) AS Tutari_Net " + 
                              ...
                              "INNER JOIN Islem_Detay_Stok AS IDS ON IB.Silindi = IDS.Silindi AND IB.Kurum_Kodu = IDS.Kurum_Kodu AND IB.Fis_Tipi = IDS.Fis_Tipi AND IB.Isyeri_Kodu = IDS.Isyeri_Kodu AND IB.Fis_No = IDS.Fis_No " +
                              "INNER JOIN Depo_Tanitimi AS DT ON IB.Kurum_Kodu = DT.Kurum_Kodu AND IB.Depo_Kodu_1 = DT.Depo_Kodu " +
                              "WHERE IB.Silindi = 0 " +
                              "AND (SELECT COUNT(Irsaliye_Fis_No) FROM Stok_Bagli_Irsaliyeler WHERE Kurum_Kodu = IB.Kurum_Kodu AND Irsaliye_Fis_Tipi = IB.Fis_Tipi AND Irsaliye_Isyeri_Kodu = IB.Isyeri_Kodu AND Irsaliye_Fis_No = IB.Fis_No AND (Fatura_Fis_Tipi <> @Fatura_Fis_Tipi OR Fatura_Isyeri_Kodu <> @Fatura_Isyeri_Kodu OR Fatura_Fis_No <> @Fatura_Fis_No)) = 0 " +
                              "AND IB.Kurum_Kodu = @Kurum_Kodu AND IB.Fis_Tipi = @Fis_Tipi " + 
