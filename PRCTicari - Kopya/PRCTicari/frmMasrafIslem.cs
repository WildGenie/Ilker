...
namespace PRCTicari
{
    public partial class frmMasrafIslem : Form
    {
        int intFisTipi = 0;
        ...
        public frmMasrafIslem(clsFisTipleri.FisTipleri ftGetFisTipi)
        {
            InitializeComponent();
        ...
        private void frmMasrafIslem_Shown(object sender, EventArgs e)
        {
            blnDegisti = false;
        ...
        public void prcdTemizle(bool blnMaxFisNo = true)
        {
            ...
            txtToplamBorc.Clear();
            txtCariKodu.Clear();
            txtKasaKodu.Text = clsGenel.fncGetParameter("Kasa_Kodu");
            intCariNo = 0;
                ...
                if (cbIsyeriKodu.SelectedItemForCode().TOINT() != 0)
                {
                    if (txtFisNo.Text.Trim().TOINT() != 0)
                    {
                        ...
                        cmd.CommandText = "SELECT IB.*, CT.Cari_Kodu " +
                                          ...
                                          "WHERE IB.Kurum_Kodu = @Kurum_Kodu AND IB.Fis_Tipi = @Fis_Tipi AND IB.Isyeri_Kodu = @Isyeri_Kodu AND IB.Fis_No = @Fis_No";
                        ...
                        cmd.CommandText = "SELECT ID.Masraf_No, MT.Masraf_Kodu, '...' AS Sec_Masraf_Kodu, MT.Masraf_Adi, ID.Kdv_Orani, ID.Kdv_Tutari, ID.Alacak_Tutari, ID.Borc_Tutari, ID.Aciklama, MT.Masraf_Tipi " +
                                          "FROM Islem_Detay_Masraf AS ID " +
                                          "LEFT OUTER JOIN Masraf_Tanitimi AS MT ON MT.Silindi = 0 AND MT.Kurum_Kodu = ID.Kurum_Kodu AND MT.Masraf_No = ID.Masraf_No " +
                                          "WHERE ID.Kurum_Kodu = @Kurum_Kodu AND ID.Fis_Tipi = @Fis_Tipi AND ID.Isyeri_Kodu = @Isyeri_Kodu AND ID.Fis_No = @Fis_No ";
                        cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                        cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                        cmd.Parameters.AddWithValue("@Isyeri_Kodu", cbIsyeriKodu.SelectedItemForCode().TOINT());
                        cmd.Parameters.AddWithValue("@Fis_No", txtFisNo.Text.Trim().TOINT());
        ...
        private void dgvKalemler_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            dgvKalemler.Columns[e.ColumnIndex].Name == colKdv.Name)
            {
                if (dgvKalemler.Columns[e.ColumnIndex].Name == colBorcTutari.Name)
        ...
        private void dgvKalemler_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (dgvKalemler.Columns[e.ColumnIndex].Name == colSecMasrafKodu.Name)
                {
        ...
        private void prcdMasrafBilgiGetir()
        {
            ...
            cnn.Open();
            SqlCommand cmd = cnn.CreateCommand();
            cmd.CommandText = "SELECT * FROM Masraf_Tanitimi WHERE Islem = 1 AND Kurum_Kodu = @Kurum_Kodu AND Masraf_Kodu = @Masraf_Kodu";
            cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
            cmd.Parameters.AddWithValue("@Masraf_Kodu", dgvKalemler.CurrentRow.Cells[colMasrafKodu.Name].Value);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                dgvKalemler.CurrentRow.Cells["colMasrafNo"].Value = reader["Masraf_No"];
                dgvKalemler.CurrentRow.Cells["colSecMasrafKodu"].Value = "...";
                dgvKalemler.CurrentRow.Cells["colMasrafAdi"].Value = reader["Masraf_Adi"];
                dgvKalemler.CurrentRow.Cells["colKdv"].Value = reader["Kdv_Orani"];
                dgvKalemler.CurrentRow.Cells["colMasrafTipi"].Value = reader["Masraf_Tipi"];
            }
            else
            {
                dgvKalemler.CurrentRow.Cells["colMasrafNo"].Value = 0;
                dgvKalemler.CurrentRow.Cells["colSecMasrafKodu"].Value = "...";
                dgvKalemler.CurrentRow.Cells["colMasrafAdi"].Value = "";
                dgvKalemler.CurrentRow.Cells["colKdv"].Value = 0;
                dgvKalemler.CurrentRow.Cells["colMasrafTipi"].Value = 0;
            }
            reader.Close();
            cmd.Dispose();
            cnn.Close();
        ...
        private void tsbSil_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bu fişi silmek istediğinize emin misiniz?", "Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                try
                {
                    cmd.CommandText = "UPDATE Islem_Baslik SET Silindi = @Silindi_Yeni, Delete_User = @Delete_User, Delete_Date = @Delete_Date WHERE Kurum_Kodu = @Kurum_Kodu AND Silindi = @Silindi AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    ...
                    cmd.ExecuteNonQuery();
                    cmd.Parameters.Clear();
                    cmd.CommandText = "UPDATE Islem_Detay_Masraf SET Silindi = @Silindi_Yeni WHERE Kurum_Kodu = @Kurum_Kodu AND Silindi = @Silindi AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    cmd.Parameters.AddWithValue("@Silindi_Yeni", 1);
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                    cmd.Parameters.AddWithValue("@Silindi", 0);
                    cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                    ...
                    cmd.ExecuteNonQuery();
                    cmd.Parameters.Clear();
                    cmd.CommandText = "UPDATE Islem_Detay_Cari SET Silindi = @Silindi_Yeni WHERE Kurum_Kodu = @Kurum_Kodu AND Silindi = @Silindi AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    cmd.Parameters.AddWithValue("@Silindi_Yeni", 1);
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                    cmd.Parameters.AddWithValue("@Silindi", 0);
                    cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                    ...
                    cmd.ExecuteNonQuery();
                    cmd.Parameters.Clear();
                    cmd.CommandText = "UPDATE Islem_Detay_Kasa SET Silindi = @Silindi_Yeni WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                    cmd.Parameters.AddWithValue("@Silindi_Yeni", 1);
                    cmd.Parameters.AddWithValue("@Silindi", 0);
                    cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                    cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
        ...
        private bool fncKaydet()
        {
            ...
            if (dgvKalemler.Rows.Count > 0)
                dgvKalemler.CurrentCell = dgvKalemler.Rows[0].Cells[colMasrafKodu.Name];
            ...
            if (dtKalemler.Select("Masraf_Tipi = 1 AND TRIM(Aciklama) = ''").Length > 0)
            {
                MessageBox.Show("Lütfen değişken masraflara açıklama giriniz.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                return blnReturn;
            }
            ...
            try
            {
                ...
                {
                    cbIsyeriKodu_SelectedIndexChanged(cbIsyeriKodu, new EventArgs());
                    cmd.CommandText = "INSERT INTO Islem_Baslik (Fis_Tarihi, Fis_Saati, Cari_No, Kasa_Kodu, Belge_Tipi, Belge_No, Belge_Tarihi, Para_Birimi, Aciklama, Insert_User, Inser_Date, Silindi, Kurum_Kodu, Fis_Tipi, Isyeri_Kodu, Fis_No) " +
                                      "VALUES (@Fis_Tarihi, @Fis_Saati, @Cari_No, @Kasa_Kodu, @Belge_Tipi, @Belge_No, @Belge_Tarihi, @Para_Birimi, @Aciklama, @Kullanici, @Zaman, @Silindi, @Kurum_Kodu, @Fis_Tipi, @Isyeri_Kodu, @Fis_No)";
                }
                else
                {
                    cmd.CommandText = "UPDATE Islem_Baslik SET Fis_Tarihi = @Fis_Tarihi, Fis_Saati = @Fis_Saati, Cari_No = @Cari_No, Kasa_Kodu = @Kasa_Kodu, Belge_Tipi = @Belge_Tipi, Belge_No = @Belge_No, Belge_Tarihi = @Belge_Tarihi, Para_Birimi = @Para_Birimi, Aciklama = @Aciklama, Update_User = @Kullanici, Update_Date = @Zaman " +
                ...
                double dblAlacakTutariCari = intCariNo != 0 ? dtKalemler.Compute("SUM(Borc_Tutari)", "").TODOUBLE() : 0;
                double dblBorcTutariCari = intCariNo != 0 ? dtKalemler.Compute("SUM(Alacak_Tutari)", "").TODOUBLE() : 0;
                double dblAlacakTutariKasa = !string.IsNullOrEmpty(txtKasaKodu.Text.Trim()) ? dtKalemler.Compute("SUM(Borc_Tutari)", "").TODOUBLE() : 0;
                double dblBorcTutariKasa = !string.IsNullOrEmpty(txtKasaKodu.Text.Trim()) ? dtKalemler.Compute("SUM(Alacak_Tutari)", "").TODOUBLE() : 0;
                ...
                cmd.CommandText = "DELETE FROM Islem_Detay_Kasa WHERE Silindi = @Silindi AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No = @Fis_No";
                cmd.Parameters.AddWithValue("@Silindi", 0);
                cmd.Parameters.AddWithValue("@Kurum_Kodu", clsGenel.strKurumKodu);
                cmd.Parameters.AddWithValue("@Fis_Tipi", intFisTipi);
                cmd.Parameters.AddWithValue("@Isyeri_Kodu", cbIsyeriKodu.SelectedItemForCode());
                ...
                for (int iSira = 0; iSira < dtKalemler.Rows.Count; iSira++)
                {
                    ...
                    cmd.CommandText = "INSERT INTO Islem_Detay_Masraf (Fis_Tarihi, Fis_Saati, Masraf_No, Alacak_Tutari, Alacak_Tutari_Baslik, Alacak_Tutari_Kart, Alacak_Tutari_Sistem, Borc_Tutari, Borc_Tutari_Baslik, Borc_Tutari_Kart, Borc_Tutari_Sistem, Kdv_Tutari, Kdv_Tutari_Baslik, Kdv_Tutari_Kart, Kdv_Tutari_Sistem, Kdv_Orani, Aciklama, Silindi, Kurum_Kodu, Fis_Tipi, Isyeri_Kodu, Fis_No, Fis_Sira, Islem_Yonu) " +
        ...
        {
            object o = clsXKod.fncSECFisMasraf(ftFisTipi, cbIsyeriKodu.SelectedItemForCode().TOINT());
            if (o != null)
            {
        ...
        private void btnGeri_Click(object sender, EventArgs e)
        {
            if (fncVazgec(false))
            {
                ...
                cnn.Open();
                SqlCommand cmd = cnn.CreateCommand();
                cmd.CommandText = string.Format("SELECT ISNULL({0}(Fis_No), (SELECT ISNULL({1}(Fis_No), CAST(1 AS INT)) FROM Islem_Baslik WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu_Ic AND Fis_Tipi = @Fis_Tipi_Ic AND Isyeri_Kodu = @Isyeri_Kodu_Ic)) AS Fis_No FROM Islem_Baslik WHERE Silindi = 0 AND Kurum_Kodu = @Kurum_Kodu AND Fis_Tipi = @Fis_Tipi AND Isyeri_Kodu = @Isyeri_Kodu AND Fis_No {2} @Fis_No",
                                                (((Button)sender).Tag.TOINT() == 0 ? "MAX" : "MIN"),
                                                (((Button)sender).Tag.TOINT() == 0 ? "MIN" : "MAX"),
                                                (((Button)sender).Tag.TOINT() == 0 ? "<" : ">"));
                cmd.Parameters.AddWithValue("@Kurum_Kodu_Ic", clsGenel.strKurumKodu);
        ...
        {
            double dblKdvOrani = dgvKalemler.CurrentRow.Cells["colKdv"].Value.TODOUBLE();
            double dblTutari = dgvKalemler.CurrentRow.Cells["colBorcTutari"].Value.TODOUBLE() + dgvKalemler.CurrentRow.Cells["colAlacakTutari"].Value.TODOUBLE();

            double dblKdvTutari = 0;
            dblKdvTutari = dblTutari - (dblTutari / (1 + (dblKdvOrani / 100)));
        ...
        private void dgvKalemler_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex > -1)
            {
                if (dgvKalemler.Rows[e.RowIndex].Cells[colMasrafTipi.Name].Value.TOINT() == 1 && string.IsNullOrWhiteSpace(dgvKalemler.Rows[e.RowIndex].Cells[colAciklama.Name].Value.TOSTRING().Trim()))
                {
